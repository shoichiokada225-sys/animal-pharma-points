...
import { query, queryOne, closePool } from './lib/db.js';
...
<style>
  * { box-sizing: border-box; }
  body { margin: 0; padding: 24px; font-family: -apple-system, BlinkMacSystemFont, "Hiragino Sans", "Yu Gothic", sans-serif; background: #f5f7fa; color: #1a202c; line-height: 1.6; }
  ...
  .tier-badge { display: inline-block; padding: 2px 10px; border-radius: 12px; font-size: 13px; font-weight: 600; color: white; margin-left: 8px; vertical-align: middle; }
  .balance-card { background: linear-gradient(135deg, #4f7fff 0%, #2b5fd1 100%); color: white; padding: 32px 24px; border-radius: 12px; box-shadow: 0 4px 12px rgba(79, 127, 255, 0.25); margin-bottom: 32px; }
  ...
  .footer { margin-top: 32px; padding-top: 16px; border-top: 1px solid #e2e8f0; font-size: 12px; color: #718096; line-height: 1.7; }
  .footer p { margin: 4px 0; }
  .updated { color: #4a5568; }
  @media (max-width: 480px) { body { padding: 16px; } .balance-value { font-size: 32px; } th, td { padding: 8px; font-size: 13px; } }
</style>
</head>
<body>
<div class="container">
  ...
  <table><thead><tr><th>日時</th><th>種別</th><th>ポイント</th><th>備考</th></tr></thead><tbody>${rows}</tbody></table>`}
  <div class="footer">
...
export async function exportCustomerViews() {
  const customers = await query('SELECT customer_id, customer_name, view_token, tier FROM customers ORDER BY customer_id');
  ...
  for (const c of customers) {
    const balRow = await queryOne('SELECT COALESCE(SUM(points),0) AS b FROM point_ledger WHERE customer_id = $1', [c.customer_id]);
    const balance = parseInt(balRow.b, 10);
    const history = await query(
      'SELECT type, points, note, created_at FROM point_ledger WHERE customer_id = $1 ORDER BY created_at DESC, ledger_id DESC LIMIT 50',
      [c.customer_id]
    );

    const html = renderHtml({ customer: c, balance, history, generatedAt, tier: c.tier });
    writeFileSync(path.join(outDir, `${c.view_token}.html`), html, 'utf8');
    count++;
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  exportCustomerViews().then(() => closePool()).catch(err => {
